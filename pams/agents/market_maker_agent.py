...
from pams.order import LIMIT_ORDER, Cancel
...
class MarketMakerAgent(HighFrequencyAgent):
    ...
    def submit_orders(self, markets: List[Market]) -> List[Union[Order, Cancel]]:
        ...
        orders: List[Order] = []
        price_margin: float = self.target_market.get_fundamental_price() * self.net_interest_spread * 0.5
        order_volume: int = 1
        orders.append(
            Order(
    ...
    def get_base_price(self, markets: List[Market]) -> float:
        ...
        for market in markets:
            if self.is_market_accessible(market.market_id) and market.get_best_buy_price() is float:
                max_buy = max(max_buy, market.get_best_buy_price())
        min_sell: float = float('inf')
        for market in markets:
            if self.is_market_accessible(market.market_id) and market.get_best_sell_price() is float:
